        ...
        for batch_idx, (image, target) in tqdm(enumerate(train_loader), total=len(train_loader)):
            with autocast(enabled=True):
                ...
                if np.mod(total_idx, 50) == 0:
                    print("Classification, Epoch:{}, Step:{}, Loss:{}".format(epoch, batch_idx, loss.item()))

        torch.save(model.state_dict(), 'checkpoints/clf_{}.pth'.format(str(epoch).zfill(2)))
        eval(model, extractor, val_loader, device)
...
def eval(model, extractor, val_loader, device):
    ...
    print('Validation Loss:', np.mean(loss_list), 'Validation Error:', np.mean(error_list))
